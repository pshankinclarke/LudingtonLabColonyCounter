...
def exrez():
   hw = []
   photolist = [f for f in listdir(photo_dir) if isfile(join(photo_dir, f))]
   for photo in photolist:
      if photo.endswith('.png') or photo.endswith('.JPG') or photo.endswith('.jpg'):
         if photo != 'controlSYN.png' and photo != 'controlRL.png':
            img = cv2.imread(photo_dir +'/' + photo, cv2.IMREAD_UNCHANGED)
   ...
   for photo in photolist :
      if photo.endswith('.png') or photo.endswith('.JPG') or photo.endswith('.jpg'):
         if photo != 'controlSYN.png' and photo != 'controlRL.png':
            ...
            oldimg = cv2.imread(photo_dir +'/' + photo, cv2.IMREAD_UNCHANGED)
