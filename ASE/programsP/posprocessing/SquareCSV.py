...
def exS():
   ...
   for i in range(len(photonames)):
      ...
      chunks = _cc[i]
      headings = ['photograph','row','column','loc','ratio1','ratio2','CFUs','MCH','NP','date','time','treatment','cw/rw']
      arbNumb = []
      ...
      for i in range(len(rac)):
         if string != 'controlRL.png' and string != 'controlSYN.png': 
            if i == 0 : 
               ML.append(headings)
               ML.append([string,rac[i][0],rac[i][1],rac[i],l[2],l[3],chunks[i][2],chunks[i][0],chunks[i][1],l[5],l[7],l[6],l[len(l)-1].replace('.png','')]) 
            else: 
               ML.append([string,rac[i][0],rac[i][1],rac[i],l[2],l[3],chunks[i][2],chunks[i][0],chunks[i][1],l[5],l[7],l[6],l[len(l)-1].replace('.png','')]) 
         else:
            if i == 0 :
               ML.append(['photograph','row','column','loc','CFUs','MCH','NP'])
               ML.append([string,rac[i][0],rac[i][1],rac[i],chunks[i][2],chunks[i][0],chunks[i][1]])
            else:
